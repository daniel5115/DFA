...
public class Automata extends JFrame implements ActionListener,ItemListener {
	...
	int x;//numero de estados que habra,se le preguntara al usuario en la quintupla
	State[] statesAll;//max 5 states
	...
	String st;//donde se almacenara el string que
	char[]arregloString;//descomposicion del String
	int[] arrayString;//guardara el String descompuesto en 01 .
	int[][] tablatransicion;//donde se guardara la matriz de transicion
	int[]transicion;//se almacenara la transicion del String dado
	boolean strinState;//guardara si el string fue aceptado por el automata
...
//Metodo para los estados
public void askTupla(String edos,String in,String fin) {
    ...
    finalesta=new int[Integer.parseInt(fin)];
     ...
     if(strinState==true)
     {
  		JOptionPane.showMessageDialog(null,"El String fue ACEPTADO debido a que termina en un estado de aceptacion");
	 }
	 else
	 {
	 	JOptionPane.showMessageDialog(null,"El String fue RECHAZADO debido a que NO termina en un estado de aceptaci?n");}
   		imprimirText();
	 }

  //Metodo para el ALFABETO
  ...
  //Metodo para la tabla de transición
  void transicion()
  {
        ...
        try
        {
        	...
        	for(int ii=0; ii<x; ii++)
        	{
            	...
            	{
              		String r;
                	r=JOptionPane.showInputDialog("Estado resultante  en q"+(ii+1)+" y caracter "+Character.toString(alfabeto[jj]));
...
//revisa que todo el String sea parte del alfabeto
  public boolean isCharAll()
  {
    ...
    int ab=0;
  	...
  	{
        if((arregloString[i]==alfabeto[0])||(arregloString[i]==alfabeto[1]))
        {
          	ab+=1;
        }
    }
    if(ab==arregloString.length)
    {
        a=true;
    }
      return a;
...
//convertir en 0,1 el string
  public void convertString()
  {
     arrayString=new int[arregloString.length];
     char a= alfabeto[0];
     char b=alfabeto[1];

     for(int i=0;i<arrayString.length;i++)
     {
         if(arregloString[i]==a)
         {
           arrayString[i]=0;
		 }
         if(arregloString[i]==b)
         {
           arrayString[i]=1;
         }
...
//checa la transicion del String en el automata
...
//si termina en un estado de aceptacion
      ...
      arregloString=a.toCharArray();
			...
			int cont=0;
				...
				for(int i=0;i<x;i++)
				{
					for(int j=0;j<2;j++)
					{
							...
							{
								automataTa.append("        q"+(tablatransicion[i][j]+1));
							}
		...
		public void printMoveString()
		{
			for(int i=0;i<arregloString.length;i++)
			{
						if(i==0)
						{
							automataTa.append("{q"+(initial+1)+","+arregloString[i]+"}=q"+(transicion[i]+1)+"\r\n");
						}
						else
						{
								automataTa.append("{q"+(transicion[i-1]+1)+","+arregloString[i]+"}=q"+(transicion[i]+1)+"\r\n");
						}
				}
		}
		...
		public void imprimirText()
		{
				...
				{
					automataTa.append("El String fue rechazado");
				}
